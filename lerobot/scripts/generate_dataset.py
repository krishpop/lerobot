...
def get_checkpoint_step_from_pretrained_policy_path(pretrained_policy_path: Path) -> int:
    if pretrained_policy_path.parent.is_symlink():
        return int(pretrained_policy_path.parent.resolve().name)
    else:
        return int(pretrained_policy_path.parent.name)
...
def main(
    ...
    episode_data = info.pop("episodes")
    hf_dataset = episode_data["hf_dataset"]
    episode_data_index = episode_data["episode_data_index"]
    ds_info = {
        "fps": 10,
        "video": False,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "datasets/{env_name}/{policy_name}/step_{checkpoint_step}/{timestamp}"
        ),
    )
    parser.add_argument(
        "overrides",
